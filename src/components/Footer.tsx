...
const Footer = () => {
  ...
  const socialLinks = useMemo(() => {
    ...
    links.push({ label: "Facebook", link: settings?.facebook || "https://www.facebook.com/profile.php?id=61594011362740", d: SOCIAL_SVG.facebook });
    links.push({ label: "Instagram", link: settings?.instagram || "https://www.instagram.com/elking_121?stkn=cndtbXk4dTJrYnBn", d: SOCIAL_SVG.instagram });
    if (settings?.tiktok) links.push({ label: "TikTok", link: settings.tiktok, d: SOCIAL_SVG.tiktok });
    if (settings?.whatsapp) {
      const waLink = settings.whatsapp.startsWith('http') ? settings.whatsapp : `https://wa.me/${settings.whatsapp.replace(/[^0-9]/g, '')}`;
      links.push({ label: "WhatsApp", link: waLink, d: SOCIAL_SVG.whatsapp });
    }
    return links;
  }, [settings]);
