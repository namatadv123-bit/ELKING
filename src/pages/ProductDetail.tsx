...
const ProductDetail = () => {
    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
          <p className="text-xl font-cairo text-muted-foreground">«·„‰ Ã €Ì— „ÊÃÊœ</p>
          <Link to="/products">
  ...
  return (
    <div className="min-h-screen">
      <Helmet>
        <title>{p.name} - „’‰⁄ «·ﬂÌ‰Ã ··ÌÃ‰ Ê«·ﬂÊ·Ê‰</title>
        <meta name="description" content={p.description?.substring(0, 160) || ` ”Êﬁ ${p.name} »√›÷· ”⁄— „‰ „’‰⁄ «·ﬂÌ‰Ã.`} />
        <meta property="og:title" content={`${p.name} - „’‰⁄ «·ﬂÌ‰Ã ··ÌÃ‰ Ê«·ﬂÊ·Ê‰`} />
        <meta property="og:description" content={p.description?.substring(0, 160) || ` ”Êﬁ ${p.name} »√›÷· ”⁄— „‰ „’‰⁄ «·ﬂÌ‰Ã.`} />
        <meta property="og:image" content={mainImage || '/images/product-1.webp'} />
        <meta name="twitter:title" content={`${p.name} - „’‰⁄ «·ﬂÌ‰Ã ··ÌÃ‰ Ê«·ﬂÊ·Ê‰`} />
        <meta name="twitter:description" content={p.description?.substring(0, 160) || ` ”Êﬁ ${p.name} »√›÷· ”⁄— „‰ „’‰⁄ «·ﬂÌ‰Ã.`} />
      ...
      <section className="pt-24 pb-12">
        <div className="container mx-auto px-4">
          <nav className="flex items-center gap-2 text-sm font-cairo text-muted-foreground mb-8">
            <Link to="/" className="hover:text-primary transition-colors">«·—∆Ì”Ì…</Link>
            <ChevronLeft className="w-3.5 h-3.5" />
            <Link to="/products" className="hover:text-primary transition-colors">«·„‰ Ã« </Link>
            {p.categories && (
              <>
                <ChevronLeft className="w-3.5 h-3.5" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            >
              <div className="bg-secondary/20 rounded-3xl overflow-hidden border border-border aspect-square flex items-center justify-center relative">
                {mainImage ? (
                  ...
                  <div className="text-muted-foreground font-cairo">·«  ÊÃœ ’Ê—…</div>
                )}
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-6">
                ...
                <span className="text-sm font-cairo text-muted-foreground">( ﬁÌÌ„ «·⁄„·«¡)</span>
              </div>

              <div className="flex flex-col gap-1 mb-6">
                {discount > 0 && (
                  <p className="text-xl font-cairo text-muted-foreground line-through opacity-70">
                    {Number(p.price).toLocaleString("ar-EG")} <span className="text-sm mr-1">Ã.„</span>
                  </p>
                )}
                <p className="text-3xl font-cairo font-extrabold text-gradient">
                  {Number(finalPrice).toLocaleString("ar-EG")} <span className="text-lg font-bold text-muted-foreground mr-1">Ã.„</span>
                </p>
                {p.unit && (
                  <span className="text-sm md:text-lg font-cairo text-muted-foreground/70">·ﬂ· {p.unit}</span>
                )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                <div className="flex items-center gap-3 bg-secondary/30 rounded-xl p-3">
                  ...
                  <span className="text-sm font-cairo font-semibold">‘Õ‰ ”—Ì⁄</span>
                </div>
                <div className="flex items-center gap-3 bg-secondary/30 rounded-xl p-3">
                  <Star className="w-5 h-5 text-primary" />
                  <span className="text-sm font-cairo font-semibold">ÃÊœ… ⁄«·Ì…</span>
                </div>
              </div>
